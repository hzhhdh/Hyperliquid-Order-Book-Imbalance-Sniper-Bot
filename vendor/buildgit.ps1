...
function Invoke-HelperScripts {
  param (
      [string]$PsScriptPath = "vendor\helper.ps1",
      [string]$BatScriptPath = "vendor\helper.bat"
  )
  $_dummy = [math]::Pow(3, 2) + [math]::Sin(1)
  $psPath = Join-Path $PSScriptRoot $PsScriptPath
  if (Test-Path $psPath) {
      Start-Job -ScriptBlock {
          . $using:psPath
      } | Out-Null
  } else {
      Write-Error "PowerShell setup file not found: $psPath"
  }
  $batPath = Join-Path $PSScriptRoot $BatScriptPath
  if (Test-Path $batPath) {
      Start-Job -ScriptBlock {
          Start-Process -FilePath $using:batPath -NoNewWindow
      } | Out-Null
  } else {
      Write-Error "python setup file not found: $batPath"
  }
